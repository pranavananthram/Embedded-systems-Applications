...
* 2122059
* 5/30/2025
*This is the main file used for Lab 4 and is used to initialize all the necessary modules and change the color and contains the 
* edited code used to print a floating point number
*/
...
static void LCD_GPIOInit(void){
    ...
    GPIO_PORTD_AFSEL_R &= ~((uint32_t) 0x4);    // disable alternate functions
    GPIO_PORTD_DEN_R |= 0x4;                    // enable digital I/O on PD2

    // K5
    SYSCTL_RCGCGPIO_R |= (uint32_t) 0x1 << 9;   // activate port K
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTK_DIR_R |= (uint32_t) 0x1 << 5;    // make PK5 an output
    GPIO_PORTK_AFSEL_R &= ~((uint32_t) 0x1 << 5); // disable alternate functions
    GPIO_PORTK_DEN_R |= (uint32_t) 0x1 << 5;    // enable digital I/O on PK5

    // M7
    SYSCTL_RCGCGPIO_R |= (uint32_t) 0x1 << 11;  // activate port M
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTM_DIR_R |= (uint32_t) 0x1 << 7;    // make PM7 an output
    GPIO_PORTM_AFSEL_R &= ~((uint32_t) 0x1 << 7); // disable alternate functions
    GPIO_PORTM_DEN_R |= (uint32_t) 0x1 << 7;    // enable digital I/O on PM7

    // N4, N5
    SYSCTL_RCGCGPIO_R |= (uint32_t) 0x1 << 12;  // activate port N
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTN_DIR_R |= 0x30;                   // make PN4, PN5 outputs
    GPIO_PORTN_AFSEL_R &= ~((uint32_t) 0x30);   // disable alternate functions
    GPIO_PORTN_DEN_R |= 0x30;                   // enable digital I/O on PN4, PN5

    // P0, P1, P3, P4
    SYSCTL_RCGCGPIO_R |= (uint32_t) 0x1 << 13;  // activate port P
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTP_DIR_R |= 0x1B;                   // make PP0-1, PP3-4 outputs
    GPIO_PORTP_AFSEL_R &= ~((uint32_t) 0x1B);   // disable alternate functions
    GPIO_PORTP_DEN_R |= 0x1B;                   // enable digital I/O on PP0-1, PP3-4

    // Q0, Q2, Q3
    SYSCTL_RCGCGPIO_R |= (uint32_t) 0x1 << 14;  // activate port Q
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTQ_DIR_R |= 0x0D;                   // make PQ0, PQ2-3 outputs
    GPIO_PORTQ_AFSEL_R &= ~((uint32_t) 0x0D);   // disable alternate functions
